\documentclass[12pt]{report}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ESTILOS
\usepackage{xcolor}

% tablas
\usepackage{booktabs}  % Para líneas horizontales mejoradas
\usepackage{array}     % Para ajustar el espacio vertical en las celdas
\usepackage{rotating}  % Para rotar tablas
\usepackage{adjustbox} % Para ajustar texto a casillas
\usepackage{colortbl}  % PAra colorear celdas
\usepackage{longtable}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


% Paquetes LaTeX y estilos globales
\input{etc/pkgs}
\input{etc/style}

% Comienzo del documento
\begin{document}

    % Portada y secciones no numeradas
    \input{sections/00_portada}
    \input{sections/00_agradecimientos}
    \input{sections/00_resumen}
    \input{sections/00_abstract}
    
    % Índice del documento y de figuras
    \begingroup
        % Los enlaces son normalmente azules, pero en los índices se configuran a negro
        % para que no aparezca todo azul
        \hypersetup{linkcolor=black}
        \tableofcontents
        \listoffigures
        \lstlistoflistings
    \endgroup
    
    % Cambia el estilo de números de página a normal
    \clearpage\pagenumbering{arabic}
    
    % Capítulos del trabajo
    \input{sections/01_introduccion}
    \input{sections/02_Gestion}
    \input{sections/03_GestionDeConfiguracion}
    \input{sections/04_Requerimientos}
    \input{sections/05_Diseno}
    \input{sections/06_Implementación_Test}
    \input{sections/07_Operacion}
    \input{sections/08_Trabajos_Futuros}
    \input{sections/XX_Conclusiones}
    \appendix
    \input{sections/db_dict}
    \input{sections/matriz_entidades}
    \input{sections/nginx_conf}
    \input{sections/endpoint_forms}
    \input{sections/env_deploy}
    \input{sections/test_unitario} 

    % Bibliografía
    \bibliographystyle{plainnat}
    \bibliography{bibliografia.bib}


% Fin del documento
\end{document}
