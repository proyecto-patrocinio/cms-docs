...
\begin{longtable}{|p{1cm}|p{2.5cm}|p{12cm}|}
    ...
    \begin{itemize}
        ...
        \newline
        \item \textbf{When} Se crea la consulta ``Garantía'' con Cliente ``32165498'', oponente ``Samsung'' y descripcion ``Dummy''.
        \newline
        \item \textbf{Then} La consulta ``Garant\'ia'' para el cliente con DNI ``32165498'' deberı\'ia existir en base de datos.
        \item \textbf{And} El ticket ``Garant\'ia'' deber\'ia estar visible en el panel de entrada de la pizarra ``CONSULTANCY''.
        \item \textbf{And} La información de la consulta ``Garant\'ia'' deberı\'ia contener el cliente con DNI ``32165498''.
    ...
    \begin{itemize}
        ...
        \item \textbf{And} Existe una consulta con tag, DNI del cliente, oponente, descripción y estado:
        \begin{verbatim}
    ...    Garantía1    32165498    Samsung
    ...    Dummy    TODO     CREATED
        \end{verbatim}
        \item \textbf{And} Existe una consulta con tag, DNI del cliente, oponente, descripción y estado:
        \begin{verbatim}
        ...    Garantía2    32165498    Samsung
        ...    Dummy    TODO     CREATED
        \end{verbatim}
    ...
    \begin{itemize}
        \item \textbf{Given} Existe un usuario registrado activo con permisos ``common'' y ``case\_taker'' en la DB.
        \item \textbf{And} Existe el cliente en la base de datos:
        \begin{verbatim}
...    Romina    Cugat    DOCUMENT    32165498
...    FEMALE    1996-06-23
...    "Av Poeta Lugones 12"    5012    SINGLE
...    HOUSE    COMPLETE_UNIVERSITY
...    romina96@gmail.com
        \end{verbatim}
        \item \textbf{And} Existe el cliente en la base de datos:
        \begin{verbatim}
...    Pedro    Cugat    DOCUMENT    22145685
...    FEMALE    1980-06-25
...    "Av Poeta Lugones 12"    5012    SINGLE
...    HOUSE    COMPLETE_UNIVERSITY   pedro80@gmail.com
        \end{verbatim}
        \item \textbf{And} Se accedió a la plataforma como usuario ``Tomador de Caso''.
        ...
        \begin{verbatim}
...    32165498    Romina    Cugat    Document
...    Female    1996-06-23
...    Av Poeta Lugones 12    5,012    Single    House
...    Complete University   romina96@gmail.com
        \end{verbatim}
        \item \textbf{And} la tabla debería contener el cliente:
           \begin{verbatim}
...    22145685    Pedro    Cugat    Document    Female
...    1980-06-25  Av Poeta Lugones 12    5,012    Single
...    House  Complete University   pedro80@gmail.com
       \end{verbatim}
        \item \textbf{When} Se descarga el csv de la tabla.
        \item \textbf{Then} El archivo se debería haber descargado correctamente.
        \item \textbf{And} El archivo de clientes descargado debería ser el esperado 'expected\_clients.csv'.
        \item \textbf{When} Se crea el filtro ``First Name'' con ``Romina''.
        \item \textbf{Then} La tabla debería contener 1 filas.
        \item \textbf{And} La tabla debería contener el cliente:
        \begin{verbatim}
...    32165498    Romina    Cugat    Document
...    Female    1996-06-23  Av Poeta Lugones 12
...    5,012    Single    House   Complete University
...    romina96@gmail.com
       \end{verbatim}
       \item \textbf{When} Se descarga el csv de la tabla.
       \item \textbf{Then} El archivo se debería haber descargado correctamente.
...
.
    ...
    \hline
     PAT-SYS-13 & Realizar cambios de una consulta como usuario Profesor & 
        \begin{itemize}
        \item \textbf{Given} Existe el board ``Comisión A1'' en la DB.
        \item \textbf{And} Existe un usuario registrado activo con permisos ``common'' y ``professo'' en la DB.
        \item \textbf{And} El usuario profesor tiene acceso al board ``Comisión A1''.
        \item \textbf{And} Existe un cliente con DNI ``32165498'' en la base de datos.
        \item \textbf{And} Existe un panel llamado ``Panel A1'' para el board de la comisión ``Comisión A1''.
        \item \textbf{And} Existe un ticket para el panel, de la comisión, con tag, DNI del cliente, oponente, descripción y estado:
        ...
        \item \textbf{And} La información de la consulta ``CODE-123: Divorcio'' deber\'ia contener el cliente con DNI ``32165498''.
    ...
    \begin{itemize}
        ...
        \newline
        \item \textbf{Then} El cliente con DNI ``42301452'' deber\'ia existir en DB.
        \newline
        \item \textbf{When} Se edita el campo ``family.partner\_salary'' a ``123'' del cliente con DNI ``42301452''.
        \newline
        \item \textbf{Then} El campo ``partner\_salary'' del cliente con DNI ``42301452'' deber\'ia ser ``123'' en DB.
        \newline
        \item \textbf{When} Se elimina el cliente con DNI ``42301452''.
        \newline
        \item \textbf{Then} El cliente con DNI ``42301452'' NO deber\'ia existir la DB.
    \end{itemize}
    \\
    \hline
